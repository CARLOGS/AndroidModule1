...
class LissetConstrainLayoutActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        enableEdgeToEdge()
        setContentView(R.layout.activity_lisset_constrain_layout)
        ViewCompat.setOnApplyWindowInsetsListener(findViewById(R.id.main)) { v, insets ->
            val systemBars = insets.getInsets(WindowInsetsCompat.Type.systemBars())
            v.setPadding(systemBars.left, systemBars.top, systemBars.right, systemBars.bottom)
            insets
        }
        val cbCard = findViewById<CheckBox>(R.id.cbCard)
        cbCard.setOnCheckedChangeListener { buttonView, isChecked ->
            val clForm = findViewById<ConstraintLayout>(R.id.clForm)
            if(isChecked){
                clForm.visibility = View.VISIBLE
            }
            else{
                clForm.visibility = View.INVISIBLE
            }
        }
